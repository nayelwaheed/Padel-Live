...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", async () => {
    try {
      const reg = await navigator.serviceWorker.register("./sw.js?v=2.8.3", { scope:"./", updateViaCache:"none" });
      try { await reg.update(); } catch (_) {}
      if ("Notification" in window) {
...
renderAll();
refreshRemote();
if (localStorage.getItem("padel-live-background-push-enabled") === "true") {
  initOneSignal();
} else {
  updateBackgroundAlertUI();
}
