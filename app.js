...
let oneSignalListenerAttached = false;
...
async function initOneSignal() {
  ...
  if (oneSignalInitialized && oneSignalInstance) return true;
  ...
  oneSignalInitPromise = new Promise(resolve => {
    OneSignalDeferred.push(async function(OneSignal) {
      try {
        ...
        // OneSignal v16 must be initialized only once per page.
        // If the SDK has already been initialized by an earlier callback,
        // reuse that live SDK instance instead of failing the whole setup.
        try {
          ...
          console.info("OneSignal was already initialized; reusing the existing SDK instance.");
        }
        ...
        if (!oneSignalListenerAttached) {
          OneSignal.User.PushSubscription.addEventListener("change", ev => {
            oneSignalReady = Boolean(ev.current?.optedIn);
            if (oneSignalReady) {
              localStorage.setItem("padel-live-background-push-enabled", "true");
            }
            updateBackgroundAlertUI();
            queuePreferenceSync();
          });
          oneSignalListenerAttached = true;
        }
...
async function enableBackgroundAlerts() {
  ...
  const initialized = await if (localStorage.getItem("padel-live-background-push-enabled") === "true") {
  initOneSignal();
} else {
  updateBackgroundAlertUI();
}
  ...
  try {
    ...
    if (!OneSignal.Notifications.permission) {
      setBackgroundStatus("Waiting for notification permission…");
      await OneSignal.Notifications.requestPermission();
    }

    setBackgroundStatus("Enabling background alerts…");
    ...
    if (!oneSignalReady) {
      setBackgroundStatus("Push permission was not enabled");
      return;
    }
    ...
    console.error("Could not enable background push", e);
    setBackgroundStatus(`Could not enable · ${String(e?.message || e).slice(0,40)}`);
  }
}
...
async function refreshRemote({ manual = false } = {}) {
  ...
  try {
    const apiUrl = `${base}/api/data?_=${Date.now()}`;
    ...
    console.warn("Padel Live refresh failed", { error: err, base: cfg.apiBaseUrl });
    state.remoteConnected = false;
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", async () => {
    try {
      const reg = await navigator.serviceWorker.register("./sw.js?v=2.8.2", { scope:"./", updateViaCache:"none" });
      try { await reg.update(); } catch (_) {}
      if ("Notification" in window) {
