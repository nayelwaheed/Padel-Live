...
async function requestAlerts(test = false) {
  ...
  // iPhone/iPad Web Push is available to Home Screen web apps, not a normal Safari tab.
  ...
  try {
    setNotificationStatus("Checking service worker…");
    ...
    if (test) {
      ...
      // Use a non-blocking visual confirmation; iOS may place the banner in
      // Notification Center while the web app itself is in the foreground.
      const oldTitle = document.title;
      document.title = "✓ Notification sent";
      setTimeout(() => { document.title = oldTitle; }, 2200);
    }
  } catch (e) {
    console.error("Padel Live notification test failed:", e);
    setNotificationStatus(`Test failed: ${e?.name || "unknown error"}`);
    alert(
      "The notification test failed. Make sure you opened Padel Live from its Home Screen icon. " +
      "If it is already installed, remove the Home Screen app, add it again from Safari, and retry."
    );
  }
}
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", async () => {
    try {
      ...
      setNotificationStatus("Service worker unavailable");
    }
  });
}
renderAll();
